# any returns true if at least one element of an array meets a given condition
# all returns true if all elements of an arrays meet a given condition

numbers = (1..10).to_a
puts "Does numbers have numbers between 3 and 6? R: #{numbers.any? { |n| n.between?(3,6)}}" # true
puts "Do all elements of numbers are greater than 7? R: #{numbers.all? { |n| n > 7}}" # false
...
def has_greater_than_seven_characters(strs)
    # strs is the array of strings
    strs.any? { |w| w.length > 7}
end
...
def against_all_odds(nums)
    # nums is the array of numbers
    nums.all? { |n| n.even? }
end

puts "against_all_odds"
